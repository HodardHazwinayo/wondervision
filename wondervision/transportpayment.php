...
if(isset($_REQUEST['print']))
{
	...
	$get=mysql_query("SELECT payment_id,commisionamount FROM payment_details WHERE payment_id='".$_REQUEST['pid']."'");
	
    $row = mysql_fetch_assoc($get);
	//$idmax=$row['payment_id'];
	$comperct = $row['commisionamount'];
	
	$commissionamouts = ($_REQUEST['advance'] * ($comperct/100));
	
	$transql = "INSERT INTO transaction_master(total_amount,advance_amount,remaining_amount,date,commission_amount,payment_id) VALUES ('".$_REQUEST['gtotal']."','".$_REQUEST['advance']."','".$_REQUEST['due']."','".$datetime."','".$commissionamouts."',(SELECT payment_id FROM payment_details WHERE payment_id='".$_REQUEST['pid']."'))";
...
<script>
	...
	window.location='payment.php?eid=<?php echo $_REQUEST['eid'] ?>&pid=<?php echo $_REQUEST['pid'] ?>&trid=<?php echo $_REQUEST['trid'] ?>&bid=<?php echo $_REQUEST['bid'] ?>&tid=<?php echo $_REQUEST['tid'] ?>';
		  ...
		  <form method="post">
			...
			<div style="float:left;margin:10px;"></div>
			  ...
			  <div style="float:left;width:1034px;height:330px;margin:10px 0;">
				   ...
				   <?php
					$trsql = mysql_query("SELECT * FROM transaction_master WHERE transaction_id='".$_REQUEST['trid']."' AND payment_id='".$_REQUEST['pid']."' order by transaction_id desc limit 1");
						
						$trrow = mysql_fetch_array($trsql);
				   ?>
			...
			<form method="post">	
				...
				<tr><td><h4>Total</h4></td><td>&nbsp;</td><td>&nbsp;</td>
				<td><input type="text" value="<?php echo $trrow['remaining_amount'] ?>" name="gtotal" id="gtotal" onfocus="cal_gt()" readonly></td></tr>
