...
<script>
		window.location="payment.php?eid=<?php echo $heid ?>&pid=<?php echo $idmax ?>&trid=<?php echo $trid ?>&bid=<?php echo $hbid ?>";
</script>
		  ...
		  <form method="post">
			<div id="tabs" style="float:left;height:295px;">
				<ul>
				    ...
				    <li><a href="#tabs-4">Resort</a></li>
				  </ul>
				  <div id="tabs-1">
					 <table>
					 <?php
						$hsql = "SELECT * FROM enquiry_accomodation_mapping INNER JOIN accomodation_type_details ON enquiry_accomodation_mapping.accomodation_type_id=accomodation_type_details.accomodation_type_id INNER JOIN hotel_master ON accomodation_type_details.accomodation_type_id=hotel_master.hotel_type_id WHERE enquiry_accomodation_mapping.accomodation_type_id = '".$_REQUEST['hatid']."'";
						$hrs = mysql_query($hsql);
						
						$hrow = mysql_fetch_array($hrs);
					 ?>
						<tr><td><h4>Hotel Name</h4></td><td><?php echo $hrow['name'] ?></td></tr>
						<tr><td><h4>Place</h4></td><td><?php echo $hrow['place'] ?></td></tr>
						<tr><td><h4>Postal Zip</h4></td><td><?php echo $hrow['zip'] ?></td></tr>
						<tr><td><h4>Phone Number</h4></td><td><?php echo $hrow['phonenumber'] ?></td></tr>
					</table>		
				  </div>
				  <div id="tabs-2">
			...
			<div style="float:left;height:auto;margin-left:5px;border:1px solid #AAAAAA; border-radius:5px;padding:15px;">
				...
				<table align="center">
					...
					<?php 
					$easql = "SELECT * FROM enquiry_accomodation_mapping WHERE enquiry_id='".$_REQUEST['heid']."'";
					$eares = mysql_query($easql);
					$earow = mysql_fetch_array($eares);
					?>
					<tr>
				
						<td style="padding-right:40px;" align="center"><?php echo $earow['checkindate'] ?></td>
						<td style="padding-right:40px;" align="center"><?php echo $noofrooms = $earow['noofrooms'] ?>
						<input type="hidden" id="noofrooms" value="<?php echo $noofrooms ?>">
						</td>
						<td style="padding-right:40px;" align="center">
						
						<?php   $diff = strtotime($earow['checkoutdate']) - strtotime($earow['checkindate']);
								echo $totday=floor($diff/(3600*24));
						?>
						<input type="hidden" id="totday" value="<?php echo $totday ?>">
						</td>
						<td style="padding-right:40px;" align="center"><?php echo $earow['checkoutdate'] ?></td>
					</tr>
				</table>
			</div>
			...
			<div style="float:left;margin:10px;"></div>
			  <div style="float:left;height:auto;padding:10px 80px 10px 20px;border:1px solid #aaaaaa; border-radius:5px;margin:10px 0px;">
					...
					<?php
					$usql = mysql_query("SELECT * FROM enquiry_details INNER JOIN user_master ON enquiry_details.user_id=user_master.user_id INNER JOIN user_other_info ON user_other_info.user_id=user_master.user_id WHERE enquiry_id='".$_REQUEST['heid']."'");
					...
					<tr>
						...
						<td align="center"><input type="text" value="<?php echo $earow['noofadults'] ?>" size="40" readonly></td>
						<td>&nbsp;</td><td>&nbsp;</td>
						<td align="center"><h4>Child(ren)</h4></td>
						<td align="center"><input type="text" value="<?php echo $earow['noofchildren'] ?>" size="40" readonly></td>
					</tr>
					...
					<td>
						<input type="text" value="<?php echo $earow['roomtype'] ?>" size="20" readonly>
					</td>
					<td>&nbsp;&nbsp;</td>
					<td>&nbsp;</td>
					<td>
			...
			<form method="post">	
					<tr><td><h4>Total Amount</h4></td><td>&nbsp;</td><td>&nbsp;</td><td><input type="text" name="rate_f" id="rate_f" 
					onfocus="cal_ta2()" readonly></td></tr>
